...
public class CmsLogAct {
	...
	public String listOperating(String queryUsername, String queryTitle,
			...
			ModelMap model) {
		Pagination pagination = manager.getPage(AdminLog.OPERATING, queryUsername, queryTitle, queryIp, cpn(pageNo), CookieUtils
						.getPageSize(request));
	...
	public String listLoginSuccess(String queryUsername, String queryTitle,
			String queryIp, Integer pageNo, HttpServletRequest request,
			ModelMap model) {
		Pagination pagination = manager.getPage(AdminLog.LOGIN_SUCCESS, 
	...
	public String listLoginFailure(String queryTitle, String queryIp,
		...
		Pagination pagination = manager.getPage(AdminLog.LOGIN_FAILURE, 
	...
	public String deleteOperating(String queryUsername, String queryTitle,
		...
		AdminLog[] beans = manager.deleteByIds(ids);
		for (AdminLog bean : beans) {
			log.info("delete AdminLog id={}", bean.getId());
		}
	...
	public String deleteOperatingBatch(Integer days,
			HttpServletRequest request, ModelMap model) {
		manager.deleteBatch(AdminLog.OPERATING, days);
	...
	public String deleteLoginSuccessBatch(Integer days,
		...
		manager.deleteBatch(AdminLog.LOGIN_SUCCESS,  days);
		model.addAttribute("message", "global.success");
	...
	public String deleteLoginFailureBatch(Integer days,
		...
		manager.deleteBatch(AdminLog.LOGIN_FAILURE,  days);
		model.addAttribute("message", "global.success");
	...
	private boolean vldExist(Integer id, WebErrors errors) {
		...
		AdminLog entity = manager.findById(id);
		if (errors.ifNotExist(entity, AdminLog.class, id)) {
			return true;
		}
		return false;
	}
